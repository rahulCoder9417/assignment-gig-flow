...
import { deleteGig } from '@/store/slices/gigsSlice';
...
const MyGigs = () => {
  ...
  const myGigs = useMemo(
    () => gigs.filter((g) => g.clientId === user?.id),
    [gigs, user]
  );
  ...
  const getBidCountForGig = (gigId: string) => {
    return bids.filter((b) => b.gigId === gigId).length;
  };

  const getPendingBidCount = (gigId: string) => {
    return bids.filter((b) => b.gigId === gigId && b.status === 'pending').length;
  };
  ...
  const GigRow = ({ gig, index }: { gig: typeof myGigs[0]; index: number }) => (
    <motion.div
        ...
        <CardContent className="p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {getBidCountForGig(gig.id)} bid
                        {getBidCountForGig(gig.id) !== 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {gig.status === 'open' && getPendingBidCount(gig.id) > 0 && (
                <Badge className="bg-accent text-accent-foreground">
                  {getPendingBidCount(gig.id)} pending
                </Badge>
              )}
              <div className="flex items-center gap-1">
